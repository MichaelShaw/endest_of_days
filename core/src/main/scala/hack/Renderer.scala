...
import hack.game.{Tile, Vec2i, World}
...
class Renderer {
  ...
  tileAtlas(Tile.playerAFactory.id) = tileRegion(2, 1)
  tileAtlas(Tile.playerBFactory.id) = tileRegion(3, 1)
  ...
  def renderPlayers(world : World) : Unit = {
    mainBatch.draw(tileAtlas(Tile.playerAFactory.id), world.playerA.x * tileSizeScreen, world.playerA.y * tileSizeScreen, tileSizeScreen, tileSizeScreen)
    mainBatch.draw(tileAtlas(Tile.playerBFactory.id), world.playerB.x * tileSizeScreen, world.playerB.y * tileSizeScreen, tileSizeScreen, tileSizeScreen)
  }
}
